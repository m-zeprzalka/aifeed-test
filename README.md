...
1. **Scrape** — 24 feedy RSS (`src/lib/scraper/sources.ts`) równolegle; filtr słów kluczowych AI z granicami słów.
...
4. **Pełna treść + multi-source** — `scrapeArticleContent` przez `safeFetch` (hardening SSRF — patrz §7). **Synteza źródeł**: `findRelatedItems` (parser.ts, ≥3 wspólne tokeny tytułu, inny outlet) znajduje do 2 doniesień o tym samym wydarzeniu; ich treść trafia do promptu jako "ŹRÓDŁO DODATKOWE", a artykuł jest syntezą (information gain — dominujący sygnał jakości od marcowego Core Update 2026). Zmergowane URL-e są oznaczane jako przetworzone po insercie.
5. **Generacja** — `anthropic/claude-sonnet-5` przez OpenRouter (adaptive thinking domyślnie ON; `max_tokens: 12000` mieści thinking + treść; taniej niż stary Sonnet 4: $2/$10 vs $3/$15 MTok); wynik `treść\n---META---\n{json}`; trzy strategie ekstrakcji meta; `normalizeMarkdown` + `sanitizeInternalLinks` + `polishTypography`. Prompt: target **700–1200 słów** z obowiązkowym tłem/konkretem/znaczeniem, katalog istniejących tagów, lista 40 ostatnich artykułów (1-3 kontekstowe linki wewnętrzne, sanitizer wycina resztę), „polski kąt", dywersyfikacja struktury. **Dyscyplina tagów egzekwowana też w kodzie** (3 stopnie, route.ts): tagi z top-100 przechodzą → tagi istniejące w DB przechodzą → tagi całkiem nowe: max 1; łącznie max 5. Retry ×1 na 429/5xx.
